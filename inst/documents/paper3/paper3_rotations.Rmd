...
```{r, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
  ...
  #df_cost <- df_res_all
  #TEST looking at when freq limit removed :
  load("df_res_all_rot_cost10000_no_freq_limit.rda")
  df_cost <- df_res_all
...
Dominance of resistance | sets fitness of heterozygous (SR) insects between that of SS & RR in presence of insecticide | 0 | 1
...
Dominance of cost  | sets fitness of heterozygous (SR) insects between that of SS & RR in absence of insecticide | 0 | 1
...
It is necessary to choose a measure to quantify the performance of an insecticide-use strategy and enable comparison with an alternative. For this we summed the number of generations when an insecticide was deployed and the resistance allele frequency for that insecticide was below 0.5. This allowed comparison between strategies using a variable number of insecticides.
...
1. *add to methods description of how fitness determined in model, move & modify fig 1*
1. *add caveat about polygenic resistance*
1. *talk about cross resistance*
...
linputs$dom_cos <-  1
linputs$rr <-       0.5
linputs$male_expo_prop <-  0.5
linputs$plot <-     FALSE
#5/7/18 try turning off
...
ggrot <- rot_plot_resistance(dfrot, plot_refuge=FALSE, logy=TRUE, plot=FALSE, title='A. Rotation')
ggseq <- rot_plot_resistance(dfseq, plot_refuge=FALSE, logy=TRUE, plot=FALSE, title='B. Sequence')
library(gridExtra)
...
ggrot <- rot_plot_resistance(dfrot, plot_refuge=TRUE, logy=TRUE, plot=FALSE, title='A. Rotation')
ggseq <- rot_plot_resistance(dfseq, plot_refuge=TRUE, logy=TRUE, plot=FALSE, title='B. Sequence')
library(gridExtra)
grid.arrange(ggrot, ggseq, ncol=1)
...
df_all <- dplyr::bind_rows(df_nocost_nomig,
                           ...
                           df_cost_mig)
#order for ggplot
df_all$scenario <- factor(df_all$scenario, levels=c('no cost or dispersal','costs','dispersal','costs and dispersal'))
...
# 1 no cost or dispersal          both    47
#  2 no cost or dispersal       neither   952
#  3 no cost or dispersal sequence only     1
#  4                costs          both  3841
#  5                costs       neither  5948
#  6                costs rotation only   209
#  7                costs sequence only     2
#  8            dispersal          both  1938
#  9            dispersal       neither  8046
# 10            dispersal rotation only    12
# 11            dispersal sequence only     4
# 12  costs and dispersal          both  6587
# 13  costs and dispersal       neither  3250
# 14  costs and dispersal rotation only   161
# 15  costs and dispersal sequence only     2

#hack to add subplot labels to facets
abcd <- data.frame(label = c("A","B","C","D"),
...
ggplot( df_all, aes(x=success)) + 
        ...
        facet_wrap(~scenario, nrow=1) + #, scales='free_y') + #can remove free_y later if I do 10k nocost runs    
...
```{r Fig5xtra, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=9, fig.cap="Figure 5xtra. How model runs classed as success under the costs only scenario for the different strategies differ in the values of inputs. Violin plots showing how the density of input values changes according to the classification of success. Runs in which both strategies were successful were more likely to have higher costs and lower effectiveness, exposure and resistance restoration. Runs in which neither strategy was successful were the reverse. Runs for which rotations only were successful tended to have higher dominance of cost, lower dominance of resistance and lower rotation intervals. Remember that as shown in the previous figure the proportion of runs for which rotation alone succeeded was lower than for the other options."}
...
inputs <- c('cost', 'expo_hi', 'eff', 'rr', 'dom_sel', 'dom_cos','start_freqs','rot_interval','male_expo_prop')
...
#filter cost from all because the success calc was done on that
df_cost_long <- filter(df_all,scenario=='costs')
#first just select the columns I want
df_cost_long <- select(df_cost_long,c(inputs,'success'))
#gather data to be longer
df_cost_long <- tidyr::gather(df_cost_long, key=input_name, value=input_value, -success)

#there were just an odd couple of results where seq better, remove them here or earlier
df_cost_long <- filter(df_cost_long, success != 'sequence only')

ggplot(df_cost_long, aes_string(x='success',y='input_value', color='success')) + 
        #ylim(0,450) +
...
```{r Fig5xtra_cost_dispersal, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=9, fig.cap="Figure 5xtra cost and dispersal. Probably don't show in the paper. Similar to previous one. Just adds coverage and migration."}


#TODO improve labels for these

#coverage and migration added for costs and dispersal
inputs <- c('cost', 'expo_hi', 'eff', 'rr', 'dom_sel', 'dom_cos','start_freqs','rot_interval','male_expo_prop','coverage','migration')


#filter cost from all because the success calc was done on that
df_cost_long <- filter(df_all,scenario=='costs and dispersal')
#first just select the columns I want
df_cost_long <- select(df_cost_long,c(inputs,'success'))
#gather data to be longer
df_cost_long <- tidyr::gather(df_cost_long, key=input_name, value=input_value, -success)
...
```{r Fig6, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=4, fig.show='hold', fig.cap="Figure 6. Effect of cost and dispersal on the difference between rotations and sequences from model runs. The y axis shows the number of model runs for the different categories of difference between rotations and sequences. A) when there are no resistance costs or dispersal from untreated refugia, rotations and sequences produce the same results. B),D) when costs are added there are infrequent model runs where rotations last longer below resistance thresholds than sequences. For between 6 and 7% of runs including costs, rotations last more than 20% longer than sequences. C) when dispersal from untreated refugia is included without costs there are even less frequent runs where rotations last longer than sequences (in just 4 out of 10000 runs rotations lasted greater than 20% longer than sequences)."}

  # remove runs where thresholds not reached for seq & rot
  # with cost_mig ~ 65% of runs don't reach thresholds
  # could get this func to output percent not reaching
  # now don't remove the runs where thresholds not reached
  ...
  # LEFT IN FOR NOW BECAUSE used in later chunks
...
#now that non threshold runs are not removed :
#which(df_cost$pcent_rot_adv > 200)
#987 1023 1324 1365 1511 2051 2717 3187 3196 3582 3589 3768 4128 4219 4835 4880 6233 6538 6913 8666 9361
scenarios_to_plot <- c(2717)
...
```{r Fig8old, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=9, fig.show='hold', fig.cap="Figure 8. Which model inputs have the greatest influence on the difference between rotations and sequences ? In this case for scenario 2 where resistance costs are included but dispersal is not. Measured using Partial Rank Correlation Coefficients (PRCC). Positive values indicate a positive influence on the benefit of rotations, only cost and dominance of cost in this case. Dominance of cost and dominance of selection have the greatest magnitude of influence."}

#eval=FALSE because replaced by all scenarios in next chunk  
...
```{r Fig8, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=9, fig.show='hold', fig.cap="Figure 8. Which model inputs have the greatest influence on the difference between rotations and sequences ? For each scenario of resistance costs and dispersal. Measured using Partial Rank Correlation Coefficients (PRCC). Positive values indicate a positive influence on the benefit of rotations. In Resistance cost comes out as a negative impact on benefit of rotations. This could be because higher cost runs tend to be equal because they both sequences and rotations don't reach resistance thresholds. Maybe the PRCC is confusing ? Maybe Fig 5extra is clearer ?"}
...
#in_cols <- c('cost','dom_cos','dom_sel','rr','expo_hi','male_expo_prop','eff','start_freqs','coverage','migration')
in_cols <- c('start_freqs','eff','expo_hi','male_expo_prop','rr','dom_sel','cost','dom_cos','coverage','migration')
in_names <- c('start frequency','effectiveness','exposure','male exposure','resistance restoration','dominance of selection','cost','dominance of cost','coverage','migration')

scenarios <- levels(df_all$scenario) #"no cost or dispersal" "costs" "dispersal" "costs and dispersal"
#strat_name <- c("Sequential both","Adaptive both","Mix both")

plotlist <- list(length(scenarios))

for(scenario_num in 1:length(scenarios))
{  
  scen <- scenarios[scenario_num]

  df_scenario <- filter(df_all, scenario==scen)
  
    
    x <- df_scenario[,in_cols]
    y <- df_scenario['pcent_rot_adv']
    ...
    #rename column 1 from 'original to PRCC
    names(to_plot)[1] <- 'PRCC'
    # sorts the ordering of x axis
    to_plot$inputs <- factor(rownames(to_plot), levels=in_cols, labels=in_names)  
    
    #just labelling axis for final plot
    if (scenario_num == length(scenarios))
    {
      axis_text_x <- element_text(angle = 45,hjust = 1, vjust = 1)
      height <- 0.9
    } else
    ...
    plotlist[[scenario_num]] <- ggplot( to_plot, aes_string(x='inputs',y='PRCC') ) +     
  ...
  plot_grid( plotlist[[1]],plotlist[[2]],plotlist[[3]],plotlist[[4]],ncol=1, rel_heights=c(1,1,1,1.6), labels='AUTO')    

```
\pagebreak

```{r FigX8, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=9, fig.show='hold', fig.cap="Figure X8. PRCC of gens_rot modified from fig8 adding rot_interval and n_insecticides"}
...
#in_cols <- c('cost','dom_cos','dom_sel','rr','expo_hi','male_expo_prop','eff','start_freqs','coverage','migration')
in_cols <- c('start_freqs','eff','expo_hi','male_expo_prop','rr','dom_sel','cost','dom_cos','coverage','migration','n_insecticides','rot_interval')
in_names <- c('start frequency','effectiveness','exposure','male exposure','resistance restoration','dominance of selection','cost','dominance of cost','coverage','migration', 'num insecticides', 'rotation interval' )
...
gg <- ggplot(df_cost, aes(x=dom_sel, y=dom_cos)) + 
     ...
     geom_point(data=filter(df_cost, pcent_rot_adv >= 20), aes(col=pcent_rot_adv), shape=19, size=1.1, alpha=0.9) +
...
df1 <- freq_fitness_simple(cos_rr = 0.05, dom_cos = 0.9, sel_rr = 0.4, dom_sel = 0.1, plot=FALSE)
# low dominance of cost, low dominance of selection
# reduced difference at the lowest frequencies
df2 <- freq_fitness_simple(cos_rr = 0.05, dom_cos = 0.1, sel_rr = 0.4, dom_sel = 0.1, plot=FALSE)
# low dominance of cost, low dominance of selection
# hmmm similar to previous - underwhelming may not want to include
df3 <- freq_fitness_simple(cos_rr = 0.05, dom_cos = 0.1, sel_rr = 0.4, dom_sel = 0.9, plot=FALSE)
# high, high
# difference between the fitness components
df4 <- freq_fitness_simple(cos_rr = 0.05, dom_cos = 0.9, sel_rr = 0.4, dom_sel = 0.9, plot=FALSE)
...
#change levels of dom_cos to get higher value upper (facet_grid(as.table=FALSE) would do same)
dfgrid$dom_cos <- factor(dfgrid$dom_cos)#, levels=c(0.9,0.1)) 
dfgrid$dom_sel <- factor(dfgrid$dom_sel)#, levels=c(0.9,0.1)) 

#hack to add subplot labels to facets
abcd <- data.frame(label = c("A","B","C","D"),
                   dom_cos = levels(factor(dfgrid$dom_cos)),
                   dom_sel = levels(factor(dfgrid$dom_sel)))
                   #fitness_component = NA)
...
ggplot(dfgrid, aes(x=freq, y=fitness_value, col=fitness_component)) +
  ...
  facet_grid(dom_cos~dom_sel, labeller = "label_both", as.table = FALSE) #+
  # geom_text(data=abcd,
  #   aes(x = -Inf, y = +Inf, label=label),
  #   #inherit.aes=FALSE,
  #   colour='black',
...
#my subplot labelling hack fails and I'm not sure why?
#initially extra facets were added for dom_sel values of 1 & 2 that aren't in the data
#now labels AC & BD appear on same plots
