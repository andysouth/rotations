...
See :
df_res_all_rot1000-diffI-lowstart.rda
...
4/12/18
Met with Jim he offered to look at parallelising.

5/12/18
making scaled down version of sensi analysis for Jim
...
~ try to fix that min_rwr_interval seems to allow switching back to insecticides

~ check that per-diem for Geneva made it to my bank
...
~ recording mortality - is now the time to try and record data in a 'tidier' way ?



# Jim suggests making flow chart
...
~ check on Rex consortium table of when rot better, what studies were they ? ag or pub health


~ check on 1000 runs with lower startfreqs and differing insecticides 
~ is something weird going on favouring rotations more when insecticides differ ?
~ yes
