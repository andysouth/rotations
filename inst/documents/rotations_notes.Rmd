...
"One possible objection to the reactive system is that it requires an efficient and
comprehensive system for testing for resistance to avoid ‘overshooting’ and
...
Curtis CF. 1987. Genetic aspects of selection for resistance. In Ford MG, Holloman DW, Khambay BPS,
Sawicki RM, eds. Combating resistance to xenobiotics. Chichester, U.K.: Ellis Horwood, 15e161.
...
Write out some text explaining the story & maybe the order of the plots. (keep num plots manageable).
...
Not sure how best to look at effect of inputs on rot_seq, rot, neither. I could bin input ranges and then look at proportions of the 3 within them ?

This would tell me how changing an input value changes the proportion of successes, failures and rot_only. Might be OK to display. Tricky to calculate, but again gets mired in the multivariate nature of the data & potential interactions. 

If I go back to using a continuous variable of generations until thresholds I lose out on the successful results.
...
Maybe reclass all 10k runs to succeed_both (2), succeed_rot (1), fail_both (0). Then see what are the best predictors of these ? Or do series of xy plots with points coloured, coloured points with all data will probably be a mess ...
...
~ try decision tree to predict rot_minus_seq from inputs
...
~ can I add figures from specific sensi_an runs ? or safer to run the model for set runs from the paper rmd.

~ how to sort library.bib ? Have a much reduced copy in rotations folder now.


think about killer plot to display difference between rot & seq generations

be careful that min_rwr_interval doesn't come back to bite me. Ian wanted it different, i.e. not going back to an insecticide that has been used recently ...

~ sensi_an_rotations1.Rmd

~ *decide on min_rwr_interval for sensi analysis
~ decide on max_gen, what was it for paper1 ?
~ one generation per month, 12 generations per year, 500 gens is ~40 years
...
Download papers on modelling oxitech
Insecticide resistance
https://doi.org/10.1093/jee/100.5.1642
https://doi.org/10.1603/029.102.0233
...
Looks like could be very interesting :
